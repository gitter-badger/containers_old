...
#
if ( $^O eq 'linux' ) {
    $TOP_PATH = '/usr/local/tpp/';  #DDS: The last '/' is important!
    $CGI_BIN = 'cgi-bin/';
    $CGI_HOME = '/tpp/cgi-bin/';  
    $HELP_DIR = '/tpp/html/'; 
    $xslt = 'nice -19 /usr/bin/xsltproc';  # disconnect dtd check (since only has web server reference name)
